...
class Mind extends Aspect {
    ...
    void initializeThemes() {
        ...
        addTheme(new Theme(<String>["Thought","Logic","Connections","Neurons","Psychics","Subconsciousness","Intuition","Sparks", "Lightning", "Electricity"])
            ...
            ..addFeature(new DenizenQuestChain("Explore the Brain", [
                new Quest("The ${Quest.PLAYER1} has finally found the lair of ${Quest.DENIZEN}. Appropriately enough for a Mind Denizen, their dungeon is a giant replica of a brain. Doors in the dungeon simulate neurons by opening and closing at seemingly random intervals to mimic brain activity."),
                new Quest("The ${Quest.PLAYER1} has spent far too many hours trying to navigate the dungeon. They raise their fist in frustration, and at the same time, a door opens. They drop their fist and attempt to enter, but the door immediately closes again. They raise their fist once more, and the door opens again, and closes again when the ${Quest.PLAYER1} drops their fist. They raise and lower their fist a bunch of times, and the door opens and closes in perfect synch. Is...is the dungeon based off of the ${Quest.PLAYER1}’s brain!?  "),
                new Quest("The ${Quest.PLAYER1} is starting to get a hang out of controlling their brain dungeon. Most of the doors can be opened by experimentally moving part of their body or using one of their senses. (The door that opened by licking one of the walls wasn’t very fun) There is only one door the ${Quest.PLAYER1} is still unable to open, which is likely ${Quest.DENIZEN}’s."),
                new DenizenFightQuest("Through some convoluted thought processes focused on thinking about ${Quest.DENIZEN}, the ${Quest.PLAYER1} manages to open the final door to ${Quest.DENIZEN}. They are relieved it didn’t take some cliche like having to unlock their subconscious or some junk like that. They begin the final fight inside their own mind.","With ${Quest.DENIZEN} defeated, the ${Quest.PLAYER1} decides to rig explosives around the dungeon and bring the whole place down. The thought that others could basically see the inside of the ${Quest.PLAYER1}’s mind was a little too creepy.","Various doors open and close across the dungeon to simulate the ${Quest.PLAYER1}’s despair in the face of ${Quest.DENIZEN}.")
            ], new DenizenReward(), QuestChainFeature.playerIsMagicalClass), Feature.LOW)
