...
class Light extends Aspect {
    ...
    void initializeThemes() {
        ...
        new DenizenFightQuest("When the ${Quest.PLAYER1} finally enters the Casino Door, the ${Quest.DENIZEN} is already rampaging. They are pissed off as fuck that they have been off screen this entire time, and blame the ${Quest.PLAYER1} for it. It's time to strife! ","The ${Quest.PLAYER1} was lucky enough to pull off a win! The ${Quest.DENIZEN} is too dead too rampage.","Bad break, the ${Quest.DENIZEN} is going to keep throwing a hissy fit.")
            ...
            ..addFeature(new DenizenQuestChain("Beat the Odds", [
                new Quest("The ${Quest.PLAYER1} finds the enormous casino that serves as ${Quest.DENIZEN}’s lair. At the center of the casino is a lift to ${Quest.DENIZEN}’s quarters that can only be activated with a key earned by winning enough ${Quest.MCGUFFIN} chips at all the casino games. The ${Quest.PLAYER1} starts with Blackjack, and wins easily by counting the cards being dealt and only betting high when the odds are good. It helps that the ${Quest.DENIZEN} minions and ${Quest.CONSORT}s aren’t smart enough to pick up on what the ${Quest.PLAYER1} is doing."),
                new Quest("The next casino game to beat is the roulette table. The ${Quest.PLAYER1} watches the ${Quest.CONSORT}s play for a couple of games. They quickly realize there’s a defect on the wheel that causes the ball to slightly favor the right side of the wheel. They bet accordingly and eventually earn enough ${Quest.MCGUFFIN} chips to move to the next casino game."),
                new Quest("The ${Quest.PLAYER1} is stuck at the the final casino game, the slot machines. The only way to win enough ${Quest.MCGUFFIN} chips is by winning the Mega Jackpot, but there aren’t any noticeable patterns or defects in the machines to take advantage of. The ${Quest.PLAYER1} puzzles over this for a while, before paying attention to who wins the Mega Jackpot. The only winners are ${Quest.DENIZEN} minions; the slots have been rigged so the house never has to pay out! The ${Quest.PLAYER1} cuts in front of a ${Quest.DENIZEN} minion at a slot machine and wins the Mega Jackpot that was meant for the minion. The minions aren’t happy about this, but they can’t complain without revealing to the ${Quest.CONSORT} customers that the game is rigged."),
                new DenizenFightQuest("The ${Quest.PLAYER1} turns in their ${Quest.MCGUFFIN} chips at the front desk for the lift key to ${Quest.DENIZEN}’s quarters. As soon the lift brings them to ${Quest.DENIZEN}’s quarters, they are immediately attacked! It seems ${Quest.DENIZEN} is not very pleased that the ${Quest.PLAYER1} found ways to cheat at all the casino games. ","${Quest.DENIZEN} has been slain, and ownership of the casino is up for grabs. The ${Quest.PLAYER1} puts the casino in the hands of slightly more competent than average ${Quest.CONSORT}s so they can continue exploring their land.","${Quest.DENIZEN} is pleased they got ${Quest.PLAYER1} out of the way, so ${Quest.DENIZEN}’s casino can become profitable again.")
        ...
        //light is a special snowflake and can have 4 themes
