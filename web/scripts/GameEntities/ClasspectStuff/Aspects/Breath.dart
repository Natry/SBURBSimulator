...
class Breath extends Aspect {
    ...
    void initializeThemes() {
        ...
        addTheme(new Theme(<String>["Zephyr", "Fans", "Windmills", "Pinwheels", "Propellers"])
            ...
            ..addFeature(new PreDenizenQuestChain("Slay the Windmills", [new Quest("“The ${Quest.PLAYER1} returns home from exploring their land, but a ${Quest.CONSORT} is waiting at the door. Shaking heavily, she tells the ${Quest.PLAYER1} that her village is being terrorized by terrible monsters. The ${Quest.PLAYER1} agrees to help, and is lead to a plain just outside the village. The ${Quest.CONSORT} points in one direction to indicate the monsters, but all the ${Quest.PLAYER1} can see are a bunch of windmills. Wait…are the windmills the monsters?! The ${Quest.CONSORT} nods her head. The ${Quest.PLAYER1} knows consorts can be pretty dumb, but this really takes the cake.” "),
            new Quest("The ${Quest.PLAYER1} whacks the bases of a few windmills with their ${Quest.WEAPON} until they collapse. The ${Quest.CONSORT}s from the village cheer and declare the ${Quest.PLAYER1} as a true hero and slayer of beasts. The ${Quest.PLAYER1} is happy to get praised, but is still a little frustrated by all of this nonsense. "),
            new Quest("Most of the windmills have been knocked over. The ${Quest.PLAYER1} starts to hit one of the last windmills, when suddenly it rears back and opens a toothy mouth in a roar. Holy Shit! It turns out windmill monsters aren’t a delusion after all! The ${Quest.PLAYER1} slays the windmill monster in a tough strife and enjoys the ${Quest.CONSORT} village’s victory party.  "),
            ], new RandomReward(), QuestChainFeature.playerIsDestructiveClass), Feature.MEDIUM)
            , Theme.HIGH);
