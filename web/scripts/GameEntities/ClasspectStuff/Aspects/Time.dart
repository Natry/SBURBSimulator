...
class Time extends Aspect {
    ...
    void initializeThemes() {
        ...
        addTheme(new Theme(<String>["Progression","Age","Change","Future","Rivers","Possibility","Flow","Streams","Inevitability","Brooks"])
            ...
            ..addFeature(new DenizenQuestChain("Stop the Time Loop", [
                new Quest("The ${Quest.PLAYER1} takes a break from adventuring and visits a little ${Quest.CONSORT} town holding a ${Quest.MCGUFFIN} festival. They walk around town talking to the various villagers and visiting the little shops, before taking a nap in the local inn. The next morning, the villagers welcome the ${Quest.PLAYER1} to join in the ${Quest.MCGUFFIN} festival that’s just starting. Wait, wasn’t the festival yesterday? The ${Quest.CONSORT}s assure the ${Quest.PLAYER1} that the festival is only being held today. "),
                new Quest("The ${Quest.PLAYER1} tries to convince the ${Quest.CONSORT}s that they were repeating the day before, but the ${Quest.CONSORT}s refused to believe. They quickly realize that the villagers aren’t just forgetful when the same thing hsppens the day after. The village appears to be stuck in a time loop They decide to fix things out of the goodness of their heart, but not before exploiting the time loop to mess with the ${Quest.CONSORT} town for a little while."),
                new Quest("After plenty of shenanigans, the ${Quest.PLAYER1} starts investigating the time loop. They discover that the ${Quest.MCGUFFIN} festival ends with an attempted ceremony to summon ${Quest.DENIZEN}. The ${Quest.CONSORT}s bungle the ceremony every time, causing time to reset. The ${Quest.PLAYER1} studies how the festival is supposed to work, and goes through several loops at once with time time travel to fix the ceremony. "),
                new DenizenFightQuest("The ${Quest.PLAYER1} has gone through so many freaking loops that they have memorized the actions of every ${Quest.CONSORT}. They follow a perfected schedule to make sure every ${Quest.CONSORT} prepares for and completes their part of the ceremony. Finally the ceremony is completed and ${Quest.DENIZEN} arrives. The ${Quest.PLAYER1} takes out their ${Quest.WEAPON} and begins the strife!","${Quest.DENIZEN} lies dead. The ${Quest.CONSORT}s are happy to be rid of them and thank the player for helping to complete the ceremony. It was their first time performing the ceremony, after all. The ${Quest.PLAYER1} rolls their eyes.","The ${Quest.CONSORT}s wake up and begin eagerly preparing for the ${Quest.MCGUFFIN} festival. It only comes once a year, after all.")
            ], new DenizenReward(), QuestChainFeature.playerIsProtectiveClass), Feature.MEDIUM)
