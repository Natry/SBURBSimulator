...
class Hope extends Aspect {
    ...
    void initializeThemes() {
        ...
        addTheme(new Theme(<String>["Meditation", "Altars", "Hymns", "Chapels", "Priests", "Angels","Belief","Hope","Faith", "Determination"])
            ...
            ..addFeature(new PostDenizenQuestChain("Kill the Angels", [
                new Quest("The ${Quest.PLAYER1} has slain ${Quest.DENIZEN}, but there’s still one thing left to do. The land is infested with terrifying creatures called angels and it’s been creeping the ${Quest.PLAYER1} out the whole time. Clearly their ultimate quest for this land must be to eliminate all the angels and free the land from their terror."),
                new Quest("Angel killing is a lot harder than the ${Quest.PLAYER1} expected, and they were already expecting it to be pretty tough. The angels have huge amounts of health, and they’re now completely pissed off over the whole exterminating them thing going on. The land is 100 times more terrifying now with swarming angels, but things are always darkest before the dawn! The ${Quest.PLAYER1} continues on their righteous quest."),
                new Quest("All of the horrible angels have finally been slain. The land has been saved from their menace. The ${Quest.PLAYER1} excitedly waits for their quest reward, but nothing happens. They go to some of the ${Quest.CONSORT} villages to ask for a reward, but they are refused. Frankly, the ${Quest.CONSORT}s would have preferred the angels remained instead of hiding in their homes while the ${Quest.PLAYER1} was waging their one person war. Maybe genocide wasn’t the right answer after all?"),
            ], new Reward(), QuestChainFeature.playerIsDestructiveClass), Feature.WAY_LOW)
            ,  Theme.HIGH);
