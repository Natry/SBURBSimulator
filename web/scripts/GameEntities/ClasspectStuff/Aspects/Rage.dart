...
class Rage extends Aspect {
    ...
    void initializeThemes() {
        ...
        addTheme(new Theme(<String>["Denial","Rejection","Resignation","Impossibilty","Awareness","Walls","Meta","Bullshit","Finality","Acceptance", "Allowance", "Frustration"])
            ...
            ..addFeature(new PostDenizenQuestChain("Stop the Narration", [
                new Quest("The ${Quest.PLAYER1} is walking down a long road, wondering when they’ll find the next town. They suddenly stop and look behind them, asking who said that. They look everywhere but can’t find where the voice is coming from. They fruitlessly demand the narration to stop."),
                new Quest("The ${Quest.PLAYER1} is having a hard time doing any of their quests while hearing every action narrated. No one else can hear the voice and some are starting to think the ${Quest.PLAYER1} is losing their mind. The ${Quest.PLAYER1} vows to find who is doing the narrating and beat the stuffing out if them. However, the ${Quest.PLAYER1} is very unlikely to end the narration. The ${Quest.PLAYER1} raises their fist at a random location in the sky and shouts we’ll see about that."),
                new Quest("The ${Quest.PLAYER1} finds themself in front of an ancient looking broadcasting tower. They wonder aloud whether the tower is broadcasting the narration. They demand the narration to be useful for a change and just tell them, but they get no answer. In a huff, they search the tower. A couple of hours later, the ${Quest.PLAYER1} find an ominous looking machine displaying pictures of the session, and text of this narration. Realizing the machine is broadcasting the narration, and getting confirmation from the narration that it is so, they immediately destroy it. They no longer hear the narration and are pleased to go back to adventuring. The ${Quest.PLAYER1} doesn’t realize they only stopped the narration being broadcasted to their ears and not the narration itself.")
            ], new RandomReward(), QuestChainFeature.playerIsSmartClass), Feature.WAY_LOW)
